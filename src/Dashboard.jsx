  ...
  if (active && payload && payload.length) {
    return (
      <div style={{ background: '#fcfcfb', border: '1px solid #e1e0d9', borderRadius: '4px', padding: '8px 12px', fontSize: '12px', color: '#0b0b0b' }}>
        ...
        {payload.map((entry, i) => (
          <div key={i} style={{ color: entry.color }}>{entry.name}: {fmt(entry.value)}</div>
        ))}
      </div>
    );
  }
...
const CustomLabel = ({ x, y, value }) => {
  ...
  return <text x={x} y={y - 8} fill="#52514e" fontSize={10} textAnchor="middle">{fmt(value)}</text>;
};
...
export default function Dashboard() {
  ...
  const fetchData = async () => {
    try {
      ...
      const sorted = [...json].sort((a, b) => a.week_start - b.week_start);
      setData(sorted);
      if (sorted.length > 0 && sorted[0].total_arr) setTotalArr(sorted[0].total_arr);
    ...
    <div style={{ padding: '24px', maxWidth: '1000px', margin: '0 auto' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '12px', marginBottom: '24px' }}>
        <div style={{ background: '#fcfcfb', border: '1px solid #e1e0d9', borderRadius: '8px', padding: '20px 24px' }}>
          <p style={{ margin: '0 0 4px 0', fontSize: '13px', color: '#52514e', fontWeight: '400' }}>Total ARR</p>
          <p style={{ margin: '0', fontSize: '36px', fontWeight: '500', color: '#0b0b0b', letterSpacing: '-0.5px' }}>
            {totalArr ? fmt(totalArr) : '$28.5M'}
          </p>
          <p style={{ margin: '4px 0 0 0', fontSize: '12px', color: '#898781' }}>Closed Won • All time</p>
        </div>
      </div>
        ...
        <>
          <ChartCard title="Opp ARR">
            <ResponsiveContainer width="100%" height={260}>
              <LineChart data={data} margin={{ top: 20, right: 20, bottom: 60, left: 10 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="#e1e0d9" vertical={false} />
                <XAxis dataKey="week_start" tick={{ fontSize: 11, fill: '#898781' }} angle={-45} textAnchor="end" height={60} interval={0} />
                <YAxis tick={{ fontSize: 11, fill: '#898781' }} tickFormatter={fmt} width={55} />
